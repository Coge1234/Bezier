package com.example.jochen.bezier.widget.elastic;
...
/**
 * Created by JoChen on 2017/5/25.
 */
...
public class ElasticBall extends Ball {
     ...
     * 0.551915024494f
     */
    private float c = 0.552284749831f;
    ...
    public void setElasticPercent(float elasticPercent) {
        mElasticDistance = elasticPercent;
    }
     ...
     * 对外公布方法， 开启动画
     *
     * @param endPoint PointF 单位是float，以float为单位进行移动而不是int
     */
    public void startElasticAnim(PointF endPoint, ElasticBallInterface elasticBallInterface) {
        mEndPoint = new ElasticBall(endPoint.x, endPoint.y, radius);
        mStartPoint = new ElasticBall(x, y, radius);
        mStatusPoint1 = new ElasticBall(x, y, radius);
        mStatusPoint2 = new ElasticBall(x, y, radius);
        mStatusPoint3 = new ElasticBall(x, y, radius);
        mStatusPoint4 = new ElasticBall(x, y, radius);
        mStatusPoint5 = new ElasticBall(x, y, radius);
        mElasticBallInterface = elasticBallInterface;
        judgeDirection();
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mElasticBallInterface != null) {
                    mElasticBallInterface.onChange(drawElasticCircle(
                            topX, topY, offsetTop, offsetTop,
    ...
    private void animStatus4() {
        ...
        if (mDirection == DIRECTION_LEFT) {
            ...
            leftX = mStatusPoint3.leftX + percent * (mEndPoint.leftX - mStatusPoint3.leftX + mElasticDistance / 2);
            //再做一次赋值，防止和终点不重合
