...
/**
 * Created by JoChen on 2017/5/23.
 */
...
public class ElasticLoader extends View {
     ...
     * 默认静态球颜色
     */
    private static final int DEFAULT_STATIC_BALL_COLOR = 0xFFccffff;
    /**
     * 默认弹性球颜色
     */
    private static final int DEFAULT_ELASTIC_BALL_COLOR = 0xFF66ff00;
    /**
     * 默认半径
     */
...
//    private List<ElasticBall> mCircles;
    ...
    public ElasticLoader(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ElasticLoader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        mElasticCircle = new ElasticBall(mCircles.get(0).x, mCircles.get(0).y+200, mRadius);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < DEFAULT_NUM; i++) {
            canvas.drawCircle(mMarginLeft + mDistanceBall * i + mRadius + 2 *
                    i * mRadius, mMarginTop + mRadius, mRadius, mPaint);
        }
    ...
    private void startAnim() {
        mElasticCircle = new ElasticBall(mMarginLeft + mDistanceBall * mPos + mRadius + 2 *
                mPos * mRadius, mMarginTop + mRadius, mRadius);
        mElasticCircle.setDuration(1000);
        PointF endPoint = new PointF(mMarginLeft + mDistanceBall * mNext + mRadius + 2 *
                mNext * mRadius, mMarginTop + mRadius);
